...
def _log(msg: str):
    """Best-effort logger that never crashes on console encoding."""
    try:
        print(msg)
...
def init_drive():
    ...
    try:
        ...
        if not creds_json:
            _log("WARNING: GDRIVE_CREDENTIALS env var not set")
            return False
        ...
        _log(f"Google Drive connected - service account: {sa_email}")
        _log(f"Folder ID: {os.environ.get('GDRIVE_FOLDER_ID','NOT SET')}")
        return True
    except json.JSONDecodeError as e:
        _log(f"ERROR: GDRIVE_CREDENTIALS is not valid JSON: {e}")
        return False
    except Exception as e:
        _log(f"ERROR: Google Drive init failed: {e}")
        return False
...
def _reconnect():
    ...
    try:
        ...
        _drive_service = build("drive", "v3", credentials=creds)
        _log("Google Drive reconnected")
        return True
    except Exception as e:
        _log(f"ERROR: Drive reconnect failed: {e}")
        return False
...
def _get_or_create_file(filename, folder_id=None):
    ...
    try:
        ...
        if files:
            ...
            _log(f"Found Drive file: {filename} ({fid})")
            return fid
        ...
        fid = f["id"]
        _file_id_cache[filename] = fid
        _log(f"Created Drive file: {filename} ({fid})")
        return fid
    except Exception as e:
        _log(f"ERROR: Drive file lookup failed for {filename}: {e}")
        return None
...
def load_from_drive(local_path, filename="tenders_db.json"):
    ...
    try:
        ...
        file_id = _get_or_create_file(filename, folder_id)
        if not file_id:
            _log(f"WARNING: No Drive file found for {filename}")
            return False
        ...
        data = buf.getvalue()
        if len(data) < 5:
            _log(f"WARNING: Drive file is empty: {filename}")
            return False
        try:
            parsed = json.loads(data)
            if filename == "tenders_db.json":
                tender_count = len(parsed.get("tenders", {}))
                _log(f"Loaded {filename} from Drive ({len(data)//1024} KB, {tender_count} tenders)")
            else:
                _log(f"Loaded {filename} from Drive ({len(data)//1024} KB)")
        except json.JSONDecodeError:
            _log(f"ERROR: Drive file is not valid JSON: {filename}")
            return False
        local_path.parent.mkdir(exist_ok=True, parents=True)
        local_path.write_bytes(data)
        return True
    except Exception as e:
        _log(f"ERROR: Drive load failed for {filename}: {e}")
        return False
...
def _get_vault_folder_id() -> str:
    ...
    try:
        ...
        _log(f"ERROR: Vault folder error: {e}")
...
def vault_list() -> list:
    ...
    try:
        ...
        _log(f"ERROR: Vault list failed: {e}")
        return []
...
def vault_download(file_id: str) -> bytes:
    ...
    try:
        ...
        _log(f"ERROR: Vault download failed: {e}")
        return b""
...
def vault_delete(file_id: str) -> bool:
    ...
    try:
        ...
        _log(f"ERROR: Vault delete failed: {e}")
        return False
