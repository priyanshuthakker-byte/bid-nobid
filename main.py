"""
Bid/No-Bid Automation v6.1 - All audit bugs fixed
FIXES:
- POST /generate-docs/{t247_id} added (was missing → 404)
- Profile save now invalidates rules cache
- Win/loss returns correct field names for frontend
- Chat page properly wired
- All orphaned routes connected to frontend via new pages
"""
...
app = FastAPI(title="Bid/No-Bid System v6.1", version="6.1")

@app.on_event("startup")
async def startup_event():
    import time
    print("Starting Bid/No-Bid System v6.1...")
...
async def root():
    ...
    return HTMLResponse("<h1>Bid/No-Bid v6.1</h1>")
...
async def diagnose():
    ...
    chk("Google Drive", lambda: ("OK","Google Drive connected","") if drive_available() else (("ERROR","GDRIVE_CREDENTIALS not set","Add env var in Render dashboard") if not os.environ.get("GDRIVE_CREDENTIALS") else ("WARN","Drive credentials set but connection failed","Check JSON format")))
    ...
    chk("Required Files", lambda: (("OK",f"All required files present","") if not [f for f in ["extractor.py","doc_generator.py","nascent_checker.py","ai_analyzer.py","excel_processor.py","prebid_generator.py","chatbot.py","gdrive_sync.py","tracker.py","nascent_profile.json","boq_engine.py"] if not (BASE_DIR/f).exists()] else ("ERROR",f"Missing: {', '.join([f for f in ['extractor.py','doc_generator.py','nascent_checker.py','ai_analyzer.py','excel_processor.py','prebid_generator.py','chatbot.py','gdrive_sync.py','tracker.py','nascent_profile.json','boq_engine.py'] if not (BASE_DIR/f).exists()])}",f"Add to GitHub")))
    ...
    chk("POA Status", lambda: ("ERROR","Hitesh Patel POA expired 31-Mar-2026","Renew POA immediately — no bid submissions possible until renewed"))

    return {"overall": overall, "timestamp": datetime.now().isoformat(), "checks": results,
            "summary": {"ok": sum(1 for r in results if r["status"]=="OK"), "warn": sum(1 for r in results if r["status"]=="WARN"), "error": sum(1 for r in results if r["status"]=="ERROR")}}
...
async def diagnose_with_ai(data: dict = Body(...)):
    ...
    if not api_key: return {"root_cause":"No Gemini API key — go to Settings first.","fixes":[],"severity":"WARN","affected_file":"config"}
...
async def import_excel(file: UploadFile = File(...)):
    if not file.filename.lower().endswith((".xlsx",".xls")): raise HTTPException(400,"Please upload an Excel file")
        ...
        for t in tenders:
            ...
            if existing:
                ...
                if not existing.get("bid_no_bid_done"):
                    existing["verdict"]=t.get("verdict"); existing["verdict_color"]=t.get("verdict_color"); existing["reason"]=t.get("reason")
                db["tenders"][tid]=existing; updated+=1
            else:
                db["tenders"][tid]=t; added+=1
        save_db(db)
        return {"status":"success","total":len(tenders),"added":added,"updated":updated,"imported":len(tenders),
                "bid":sum(1 for t in tenders if t.get("verdict")=="BID"),
                "no_bid":sum(1 for t in tenders if t.get("verdict")=="NO-BID"),"tenders":tenders}
    finally:
...
async def dashboard():
    ...
    return {"stats": {
        "total":len(tenders),"bid":sum(1 for t in tenders if t.get("verdict")=="BID"),
...
@app.get("/tenders")
async def get_all_tenders(): return {"tenders": list(load_db()["tenders"].values())}

# ══ TENDER OPS ═════════════════════════════════════════════════════════════
@app.post("/prebid-queries")
async def get_prebid_queries_post(data: dict = Body(...)): return {"queries": generate_prebid_queries(data)}

@app.get("/prebid-queries/{t247_id}")
async def get_saved_prebid_queries(t247_id: str): return {"queries": get_tender(t247_id).get("prebid_queries",[])}
...
async def process_files(files: List[UploadFile] = File(...), t247_id: str = ""):
    ...
    try:
        ...
        for upload in files:
            fname=upload.filename or "upload"; dest=Path(tmp_dir)/fname; dest.write_bytes(await upload.read())
        ...
        doc_files=[]
        for ext in ["*.pdf","*.docx","*.doc","*.txt","*.html","*.htm"]: doc_files.extend(extract_dir.rglob(ext))
        seen,unique=set(),[]
        for f in doc_files:
        ...
        if t247_id:
            ...
            db_record.update({
                ...
                "has_corrigendum":tender_data.get("has_corrigendum",False),"ai_used":ai_used,
...
# ══ GENERATE DOCS (was missing — BUG 3 FIX) ════════════════════════════════
@app.post("/generate-docs/{t247_id}")
async def generate_docs(t247_id: str):
    """Generate all submission documents for a saved tender — was 404, now fixed."""
    ...
    try:
        ...
        # Build tender data for doc generation
        tender_data = dict(tender)
        # Ensure overall_verdict is present
        ...
        generator.generate(tender_data, str(output_path))

        # Update DB with file reference
...
# ══ PROFILE (BUG 5 FIX: invalidate cache after save) ═══════════════════════
@app.get("/profile")
...
async def update_profile(data: dict = Body(...)):
    ...
    # FIXED: invalidate rules cache so reclassify uses new rules immediately
    try:
        from excel_processor import invalidate_rules_cache
        invalidate_rules_cache()
    except (ImportError, AttributeError):
        pass  # graceful — cache will expire on next classify call
    return {"status": "saved"}
...
async def get_boq(t247_id: str):
    ...
    boq=tender.get("boq"); name=tender.get("tender_name") or tender.get("brief","")
...
async def get_win_loss():
    """BUG 4 FIX: returns correct field names — total, total_won_value, win_rate 0-1, results list"""
...
async def sync_drive():
    if not drive_available(): return JSONResponse({"status":"error","message":"Google Drive not connected"},status_code=400)
    try:
...
async def drive_status():
    ...
    return {"drive_connected":drive_available(),"tenders_in_memory":len(db.get("tenders",{})),
            "db_file_exists":DB_FILE.exists(),"db_size_kb":round(DB_FILE.stat().st_size/1024) if DB_FILE.exists() else 0}
...
async def export_tenders(verdict: str = "", search: str = ""):
    try:
        ...
        for ri,t in enumerate(sorted(tenders,key=dl),2):
            days=dl(t); v=t.get("verdict",""); rf=PatternFill("solid",fgColor=vc.get(v,"FFFFFF"))
